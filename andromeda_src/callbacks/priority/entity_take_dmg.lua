...
local function MC_ENTITY_TAKE_DMG(_, entity, amount, flags, source, countdown)
	local returned = Andromeda.AndromedaTakeDmg(entity, amount, flags, source, countdown)
	if returned ~= nil then return returned end
	
	local returned = T_Andromeda.TAndromedaTakeDmg(entity, amount, flags, source, countdown)
	if returned ~= nil then return returned end
end
