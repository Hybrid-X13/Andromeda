...
function Familiar.evaluateCache(player, cacheFlag)
	...
	if player:HasCollectible(Enums.Collectibles.PLUTONIUM) then
		local plutoniumNum = player:GetCollectibleNum(Enums.Collectibles.PLUTONIUM)
		local numPlutoniums = (plutoniumNum > 0 and (plutoniumNum + boxUses) or 0)
		...
		player:CheckFamiliar(Enums.Familiars.CHARON, numPlutoniums, player:GetCollectibleRNG(Enums.Collectibles.PLUTONIUM), Isaac.GetItemConfig():GetCollectible(Enums.Collectibles.PLUTONIUM))
	end
end
...
function Familiar.postPEffectUpdate(player)
	if player:HasCollectible(Enums.Collectibles.PLUTONIUM) then return end
	
	player:CheckFamiliar(Enums.Familiars.PLUTONIUM, 0, player:GetCollectibleRNG(Enums.Collectibles.PLUTONIUM), Isaac.GetItemConfig():GetCollectible(Enums.Collectibles.PLUTONIUM))
end
