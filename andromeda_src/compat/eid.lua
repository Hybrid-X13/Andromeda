if EID then
	...
	EID:addCollectible(Enums.Collectibles.PALLAS, "↑ Damage up for each new room explored#Special rooms give higher damage#The damage bonus resets when moving to the next floor#{{CurseLost}} Grants immunity to Curse of the Lost", "Pallas")
	...
	EID:addTrinket(Enums.Trinkets.T_ANDROMEDA_BIRTHCAKE, "Your black hole deals damage to nearby enemies every second#Every 10 enemies killed by the black hole adds one charge to Singularity", "Tainted Andromeda's Cake")
	--Cards/runes
	...
	EID:addGoldenTrinketMetadata(Enums.Trinkets.T_ANDROMEDA_BIRTHCAKE, "Every 5 enemies killed adds a charge", 0, 2)
	...
	EID:addCollectible(Enums.Collectibles.PALLAS, "↑ + Урон за каждую посещённую комнату#Особые комнаты дают больше урона#Бонус сбрасывается при переходе на следующий этаж#{{CurseLost}} Даёт иммунитет к Проклятию Потерянного", "Паллада", "ru")
	...
	EID:addTrinket(Enums.Trinkets.T_ANDROMEDA_BIRTHCAKE, "Ваша чёрная дыра каждую секунду наносит ваш урон ближайшим врагам#За каждые 10 убитых врагов, Сингулярность получает заряд", "Пирожное Порченого Андромеды", "ru")
	--Карты/руны
	...
	EID:addGoldenTrinketMetadata(Enums.Trinkets.T_ANDROMEDA_BIRTHCAKE, "Каждые 5 убитых врага дают заряд", 0, 2)
	...
	EID:addBirthright(Enums.Characters.ANDROMEDA, "{{Spode}} {{ColorTransform}}Spode 변신세트{{CR}}를 즉시 획득합니다.#{{TreasureRoom}}보물방에서의 {{Collectible" .. Enums.Collectibles.GRAVITY_SHIFT .. "}}Gravity Shift 사용 시 등장하는 버려진 천체관이 특수한 구조로 변경됩니다.", "Andromeda", "ko_kr")
	...
	EID:addCollectible(Enums.Collectibles.THE_SPOREPEDIA, "{{Collectible518}} 사용 시 그 방에서 공격하는 방향으로 특수 효과 눈물을 발사하는 랜덤 패밀리어를 소환합니다.", "스포어 책", "ko_kr")
	--Passives
	...
	EID:addCollectible(Enums.Collectibles.CERES, "{{Coin}}동전, {{Bomb}}폭탄, {{Key}}열쇠 +1#{{Heart}}/{{Coin}}/{{Bomb}}/{{Key}}/{{Battery}} 종류의 픽업 드랍 시 20%의 확률로 업그레이드됩니다.#{{HolyMantleSmall}} 피격 시 소지 중인 픽업의 양에 비례한 확률로 피해를 무시하는 보호막을 1회 제공합니다.", "케레스", "ko_kr")
	EID:addCollectible(Enums.Collectibles.PALLAS, "{{CurseLostSmall}} Lost 저주에 걸리지 않습니다.#↑ 새로운 방 입장 시마다 {{DamageSmall}}공격력 +0.2#↑ 새로운 특수방 입장 시마다 {{DamageSmall}}공격력 +0.4#↑ 새로운 {{Planetarium}}/{{SecretRoom}}/{{SuperSecretRoom}}/{{Library}} 입장 시마다 {{DamageSmall}}공격력 +0.6#스테이지 진입 시 이전 스테이지에서 올라간 공격력이 사라집니다.", "팔라스", "ko_kr")
	...
	EID:addCollectible(Enums.Collectibles.CHIRON, "↑ {{EmptyHeart}}빈 최대 체력 +1#5%의 확률로 피해를 무시하며 피해를 입은 체력의 2배만큼 회복합니다.#체력이 꽉 찼을 경우 소울하트를 회복합니다.#회복되지 않을 경우 4.5%p의 확률이 추가로 증가하며 회복 시 초기화됩니다.#빨간하트 피격 시 {{DevilChanceSmall}}악마방 확률 감소를 방어해 줍니다.", "카이론", "ko_kr")
	...
	EID:addCollectible(Enums.Collectibles.STARBURST, "6%의 확률로 적 및 지형에 부딪히면 적에게 작은 피해를 주는 여러 개의 별조각으로 분열되는 공격이 나갑니다.", "스타버스트", "ko_kr")
	--Trinkets
	...
	EID:addTrinket(Enums.Trinkets.METEORITE, "{{Collectible" .. Enums.Collectibles.EXTINCTION_EVENT .. "}} 클리어하지 않은 방에서 주기적으로 메테오를 랜덤한 위치에 떨어트립니다.#{{Burning}} 메테오가 땅에 충돌하면 폭발하며 공격력 x0~x2의 피해를 주며 화상을 입힙니다.#캐릭터는 메테오와 충돌 시 폭발의 피해를 받지 않습니다.", "운석", "ko_kr")
	...
	EID:addTrinket(Enums.Trinkets.SEXTANT, "맵에 {{Planetarium}}천체관의 위치가 표시됩니다.#새로운 방 진입 시 20%의 확률로 캐릭터가 있는 방에서 2칸 이내에 있는 스테이지 구조 및 특수방을 맵에 표시합니다.", "육분의", "ko_kr")
	...
	EID:addTrinket(Enums.Trinkets.EYE_OF_SPODE, "{{TreasureRoom}} 스테이지 입장 시 보물방이 {{AbPlanetarium}}버려진 천체관으로 바뀝니다.#{{AbPlanetarium}} 버려진 천체관은 별자리 아이템 및 다른 픽업으로 구성되어 있습니다.#↑ 버려진 천체관 교체 횟수만큼 {{Planetarium}}천체관 확률 +20%", "스포드의 눈", "ko_kr")
	EID:addTrinket(Enums.Trinkets.ANDROMEDA_BIRTHCAKE, "{{Planetarium}} 천체관 아이템이 2개가 나오며 하나를 선택하면 나머지는 사라집니다.#피격 시 적에게 주는 별조각을 여러개 소환합니다.", "안드로메다의 케이크", "ko_kr")
	EID:addTrinket(Enums.Trinkets.T_ANDROMEDA_BIRTHCAKE, "블랙홀에 있는 적에게 초당 공격력 x0.5의 피해를 줍니다.#블랙홀로 적 10마리 처치할 때마다 {{Collectible" .. Enums.Collectibles.SINGULARITY .. "}}Singularity의 충전량을 1칸 충전합니다.", '"안드로메다"의 케이크', "ko_kr")
	--Cards/runes
	...
	EID:addCard(Enums.Cards.ALPHA_CENTAURI, "방 안의 아이템 및 픽업을 제거합니다.#제거한 픽업 당 50%의 확률로 Book of Virtues의 랜덤 불꽃을 소환합니다.#제거한 아이템의 퀄리티에 따라 특수 Andromeda 모드 출신의 아이템 불꽃을 소환합니다.", "센타우루스자리 알파", "ko_kr")
	...
	EID:addGoldenTrinketMetadata(Enums.Trinkets.EYE_OF_SPODE, "특수한 구조의 등장 확률 증가", 0, 2, "ko_kr")
	EID:addGoldenTrinketMetadata(Enums.Trinkets.ANDROMEDA_BIRTHCAKE, "선택형 천체관 아이템 추가 소환", 0, 2, "ko_kr")
	EID:addGoldenTrinketMetadata(Enums.Trinkets.T_ANDROMEDA_BIRTHCAKE, "충전 속도 2배", 0, 2, "ko_kr")
end
